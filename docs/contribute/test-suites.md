...
| `XFTY_Integration` | `force-app` | The classes that do real DML — `NOW` / `DEFERRED` / `.excludePrimaryIds()` / `.depthBatched()` and the bundled Providers persisting records. Sensitive to org config. | Before a commit; in CI. |
...
`.excludePrimaryIds()` / `.depthBatched()` cases). A class with two clearly different *jobs* is also split:
