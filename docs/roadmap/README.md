...
| **Serialization enrichment** — `bundle.inject(field, config)` / `injectAll`, `XFTY_InjectConfig`, standalone `XFTY_SObjectInjector` | `XFTY_BundleEnricherTest`, `XFTY_SObjectInjectorTest`, `XFTY_Ex_EnrichmentTest` (+ helper unit tests) | [use](../use/enrichment.md) | JSON round-trip; `MOCK`-only. Needs a scratch-org verify for real insert-recalculation, `Blob` and compound fields |
