...
| [The Graph, Not the Row: What Survives the Trip Off Salesforce](the-graph-not-the-row.md) | XFTY's core pattern — Provider/Template/Bundle, field-keyed relationships, ordered value passes, shared ancestors, depth-batched inserts — walked through Apex-alongside-C#, plus what genuinely doesn't port (the field-token mechanism, `RecordType`, and the platform's own testing gotchas). |

Read them in that order for the full arc: why the framework exists → why isolated
unit tests are worth the effort → how to actually write them → how much of it was
never really about Salesforce in the first place.
